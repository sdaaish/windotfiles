...
#+end_src
** Update
Creates the update script. Tangle this with =C-v C-v C-t=, =org-babel-tangle=.

#+name: update
#+begin_src powershell :tangle update.ps1 :comments org :results silent
# Run this script after export of source code blocks.
# This will update the settings file for VS Code
$settings = Get-Content settings.json|ConvertFrom-Json

Get-ChildItem -Filter my*.json |Foreach-Object {
    $MyDefaults = Get-content $_.Name|ConvertFrom-Json

    foreach ($property in $MyDefaults.psobject.properties){
        $key =  $property.name
    ...
    Remove-item $_.Name -Force | Out-Null
}
#+end_src
** Powershell
My preferred Powershell settings.
#+begin_src powershell :results output file :file mypowershell.json
$MyDefaults = @{
...
$MyDefaults|ConvertTo-Json
#+end_src

#+RESULTS:
[[file:mypowershell.json]]
** Terminal
